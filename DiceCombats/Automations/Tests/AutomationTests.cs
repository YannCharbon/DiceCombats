...
public class AutomationTests
{
    ...
    [Fact] public void AutomationConfiguration_Serializes() { var cfg = new AutomationConfiguration { ExternalApps = [new ExternalAppDefinition { Name = "Example App", BaseUrl = "http://localhost:1234" }], Bindings = [new EventActionBinding { EventType = DiceEventTypes.InitiativeNext, Actions = [new HttpRequestActionDefinition { Type = "HttpRequest", Url = "/api/example" }, new DelayActionDefinition { Type = "Delay", Milliseconds = 100 }] }] }; var s = JsonSerializer.Serialize(cfg); var d = JsonSerializer.Deserialize<AutomationConfiguration>(s); Assert.NotNull(d); Assert.NotEmpty(d!.Bindings); }
}
